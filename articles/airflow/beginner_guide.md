...
If you have encountered the terms _orchestration_  or _Apache Airflow_ and have no clue what they mean, this article will provide clarity and a basic understanding of these concepts.
...
At this point you might be asking yourself, _Why not just use cron jobs?_ Well, think of cron jobs as an alarm clock and Airflow as project manager. Cron just runs your script at a certain time with no regard for the task's dependencies. Say, you schedule `extract.py` to run at 12:00 AM and `transform.py` to run at 1:30 AM, assuming the extraction will take 20 minutes. If the data happens to be twice as large, increasing extraction time to 40 minutes, `transform.py` will still blindly run at 1:30 AM transforming incomplete data or completely failing. Airflow on the other hand, will wait for extraction to complete, to run `transform.py` as it is dependent on it, and even when the extraction script fails, due to maybe an API timeout, airflow will rerun the script according to the `retries` configured. The cron job however, will wait until the next scheduled run.
...
**2. Atomicity:** Each task should perform one defined operation. This allows modular execution and easy debugging such that when the transformation phase fails, you only have to retry the transform task, rather than re-fetching everything from the source.
...
* **DAGs are the blueprint**, they provide a map of your tasks and dependencies, ensuring no task run out of order.
