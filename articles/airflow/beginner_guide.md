...
It simply ensures tasks run in the correct order, for instance, if data transformation depends on extraction, the extraction process must run to completion first.
_What is a DAG?_ A Directed Acyclic Graph (DAG) is a model that contains all the _tasks_ to be run. It is:
