...
CREATE OR REPLACE FUNCTION billing_records(p_first_name TEXT, p_last_name TEXT)
RETURNS TABLE (bill_id VARCHAR, bill_date DATE, service_type VARCHAR, bill_status VARCHAR, total_amount NUMERIC, paid_amount NUMERIC, balance_amount NUMERIC, payment_method VARCHAR) AS $$
BEGIN
	RETURN QUERY
		SELECT b.bill_id, b.bill_date, b.service_type, b.bill_status, b.total_amount, b.paid_amount, b.balance_amount, b.payment_method
		FROM billing b
			JOIN patient p
				ON p.patient_key=b.patient_key
		GROUP BY p.patient_key, b.bill_id, b.bill_date, b.service_type, b.bill_status, b.total_amount, b.paid_amount, b.balance_amount, b.payment_method;
END;
$$ LANGUAGE plpgsql;

SELECT * FROM billing_records('John', 'Otieno');

-- Q53 Build a routine that calculates the total revenue generated by a given department.
CREATE OR REPLACE FUNCTION total_revenue_department(p_department VARCHAR)
RETURNS NUMERIC AS $$
BEGIN
	RETURN (
...
SELECT * FROM total_revenue_department('Orthopedics');


-- Q54 Create a routine that lists all appointments handled by a specific doctor.
CREATE OR REPLACE FUNCTION doctor_appointments(p_first_name TEXT, p_last_name TEXT)
RETURNS TABLE (appointment_date DATE, appointment_time TIME, patient_name TEXT, appointment_status VARCHAR, diagnosis VARCHAR) AS $$
BEGIN 
	RETURN QUERY
		SELECT a.appointment_date, a.appointment_time, CONCAT(p.first_name, ' ', p.last_name) AS patient_name, a.appointment_status, a.diagnosis 
		FROM doctor d
			JOIN appointment a
				ON d.doctor_key=a.doctor_key
					JOIN patient p
						ON a.patient_key=a.patient_key;
END;
$$ LANGUAGE plpgsql;

SELECT * FROM doctor_appointments('David', 'Otieno');


-- Q55 Develop a routine that returns all unpaid bills in the hospital.
DROP FUNCTION unpaid_bills();

CREATE OR REPLACE FUNCTION unpaid_bills()
RETURNS TABLE (patient_name TEXT, department VARCHAR, date DATE, service VARCHAR, amount NUMERIC) AS $$
BEGIN
	RETURN QUERY
		SELECT CONCAT(p.first_name, ' ', p.last_name) AS patient_name, d.department_name, b.bill_date, b.service_type, b.balance_amount 
		FROM patient p
			JOIN billing b
				ON p.patient_key=b.patient_key
					JOIN department d
						ON b.department_key=d.department_key
...
DROP FUNCTION top_doctors_appointment();

CREATE OR REPLACE FUNCTION top_doctors_appointment()
RETURNS TABLE (doctor TEXT, no_of_appointments BIGINT) AS $$
BEGIN
	RETURN QUERY
		SELECT CONCAT(d.first_name, ' ', d.last_name) AS doctor_name, COUNT(a.appointment_key)
		FROM doctor d
			JOIN appointment a
				ON d.doctor_key=a.doctor_key
		GROUP BY d.doctor_key
...
SELECT * FROM top_doctors_appointment();
