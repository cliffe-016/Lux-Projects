...
SELECT SUM(total_amount) AS total_sales_amount FROM "assignment".sales;
...
SELECT price AS highest_price FROM "assignment".products ORDER BY price DESC LIMIT 1;

-- 10. Write a query to count the number of sales for each product from the `Sales` table.
SELECT p.product_name, count(s.sale_id) AS no_of_sales 
...
SELECT c.customer_id, CONCAT(c.first_name, ' ' , c.last_name) AS full_name, SUM(s.total_amount) AS total_amount_spent
FROM "assignment".customers c 
	JOIN "assignment".sales s 
		ON c.customer_id =s.customer_id 
...
SELECT c.first_name, c.last_name, p.product_name, SUM(s.quantity_sold ) AS qty_sold
FROM "assignment".sales s
...
SELECT CONCAT(c.first_name, ' ' , c.last_name) AS full_name, p.category
FROM "assignment".customers c 
	JOIN "assignment".sales s 
		ON c.customer_id = s.customer_id 
			JOIN "assignment".products p 
...
SELECT CONCAT(c.first_name, ' ' , c.last_name) AS full_name, SUM(s.quantity_sold) AS qty_purchased
FROM "assignment".customers c 
	JOIN "assignment".sales s 
		ON c.customer_id = s.customer_id 
GROUP BY c.customer_id 		
...
SELECT p.product_name, AVG(s.quantity_sold) AS avg_qty
FROM "assignment".products p 
	JOIN "assignment".sales s 
		ON p.product_id = s.product_id 
GROUP BY p.product_id;		
...
SELECT COUNT(sale_id) AS no_of_sales
FROM "assignment".sales 
...
SELECT CONCAT(c.first_name, ' ' , c.last_name) AS full_name, SUM(s.total_amount) AS amt_spent
FROM "assignment".customers c 
	JOIN "assignment".sales s
		ON c.customer_id = s.customer_id 
WHERE EXTRACT(YEAR FROM s.sale_date) = 2023
...
FROM "assignment".products p 
	JOIN "assignment".sales s 
		ON p.product_id = s.product_id 
WHERE p.stock_quantity < 5 AND p.product_id = s.product_id;		
...
SELECT CONCAT(c.first_name, ' ' , c.last_name) AS full_name, SUM(p.stock_quantity) AS total_qty
FROM "assignment".customers c 
	JOIN "assignment".sales s
		ON c.customer_id = s.customer_id 
			JOIN "assignment".products p 
...
SELECT p.product_name , SUM(s.total_amount) AS total_sales_amount, EXTRACT(MONTH FROM s.sale_date) AS MONTH, EXTRACT(YEAR FROM s.sale_date) AS year
FROM "assignment".products p 
	JOIN "assignment".sales s 
		ON p.product_id = s.product_id 
...
SELECT CONCAT(c.first_name, ' ' , c.last_name) AS full_name, SUM(s.total_amount)  AS total_spent
FROM "assignment".customers c 
	JOIN "assignment".sales s 
		ON c.customer_id = s.customer_id 
WHERE EXTRACT(YEAR FROM s.sale_date) = 2023
...
SELECT product
FROM(
	SELECT p.product_name AS product, COUNT(s.*) AS no_of_times_sold, p.price AS price
	FROM "assignment".products p 
		JOIN "assignment".sales s 
			ON p.product_id = s.product_id
	GROUP BY p.product_name, p.price
)
WHERE price > 200 AND no_of_times_sold > 100;
...
SELECT full_name
FROM(
	SELECT CONCAT(first_name, ' ' , last_name) AS full_name, s.total_amount
	FROM "assignment".customers c 
		JOIN "assignment".sales s 
			ON c.customer_id = s.customer_id 
	GROUP BY c.customer_id, s.total_amount 
...
WITH customers AS(
	SELECT CONCAT(c.first_name, ' ' , c.last_name) AS full_name, COUNT(s.sale_id) AS count_purchases
	FROM "assignment".customers c 
		JOIN "assignment".sales s 
			ON c.customer_id = s.customer_id 
	GROUP BY c.customer_id 
...
WITH product_amount AS(
	SELECT p.product_name, SUM(s.total_amount) AS total_amount
	FROM "assignment".products p 
		JOIN "assignment".sales s 
			ON s.product_id = s.product_id
	GROUP BY p.product_id 
	),
grossing_products AS (	
	SELECT product_name
	FROM product_amount
	ORDER BY total_amount DESC
	)
SELECT * FROM grossing_products;	

-- 68. Create an intermediate result showing monthly sales totals,
--     then determine which month had the highest revenue.
WITH monthly_sales AS(
	SELECT EXTRACT(MONTH FROM sale_date) AS month, SUM(total_amount) AS total_revenue
	FROM "assignment".sales
...
WITH product_qty AS(
	SELECT p.product_id, p.product_name, COUNT(s.sale_id) AS sales_qty
	FROM "assignment".products p 
		JOIN "assignment".sales s 
			ON p.product_id = s.product_id
...
SELECT 
	sale_date,
	total_amount,
	SUM(total_amount) OVER(ORDER BY sale_date) AS running_total_sales
FROM "assignment".sales;
...
SELECT 
	sale_date,
	total_amount,
	LEAD(total_amount) OVER(ORDER BY sale_date) AS next_sale_amt
FROM "assignment".sales;
...
SELECT 
	...
	NTILE(4) OVER(ORDER BY s.total_amount DESC) AS spending_tier
FROM "assignment".sales s
	JOIN "assignment".customers c 
		ON s.customer_id = c.customer_id 
GROUP BY c.customer_id, s.total_amount;
